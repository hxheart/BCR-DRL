...
BC_SAVE_DIR = os.path.join(DATA_DIR, "bc_runs") # 这行代码定义了一个变量 BC_SAVE_DIR，它表示 BC 模型保存的目录路径。通过将 DATA_DIR 路径与子目录 "bc_runs" 组合在一起，确定了保存模型的完整路径。 定义行为克隆模型保存的目录路径
...
def train_bc_model(model_dir, bc_params, verbose=False):
    ...
    model = build_bc_model(**bc_params, max_seq_len=np.max(seq_lens), verbose=verboseio8)
