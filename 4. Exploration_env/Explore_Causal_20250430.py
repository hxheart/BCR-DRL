...
for epoch in range(epochs):
    ...
    for t in range(steps_per_epoch):
        ...
        action_logits_bc_agent = np.random.rand(5) #bc_model_train(observation_HM, training=False)        
        ...
        # observation_HM_know_AI_action = tf.reshape(obs_both_agent_know_AI_action, (1,-1)) # 得到只有 ai agent 行动以后的 HM 的 observation


        action_logits_HM_know_AI_action = np.random.rand(5) # bc_model_train(observation_HM_know_AI_action, training=False)
        ...
        reward_caci = reward_env + reward_cau
    ...
    print(f" [Causal] Epoch: ({epoch}). Mean Length: ({mean_length}).")
    print(f" Sum sparse: {sum_return_sparse}. Mean cau: {mean_return_cau}.")
    ...
    # print(f" [BCR] Epoch: {epoch}. Mean Length: {sum_length / num_episodes}")
    # print(f" Mean sparse: {sum_return_sparse / num_episodes}. ")

    # avg_return_sparse.append(sum_return_sparse / num_episodes)

    np.save('./data/data_toy_causal_sparse.npy', accumulated_sparse)
    env.render()
