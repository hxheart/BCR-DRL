...
#         reward_cau = tf.minimum(reward_env, reward_cau) # 相当于在同时没有 sparse 和 shaped 的时候，置零
    ...
    if ((epoch+1) % 20 == 0) or ((epoch+1) == epochs) :
        ...
        np.save('./data_tmp/data_ireca_return_stage.npy',  avg_return_shaped)
        np.save('./data_tmp/data_ireca_return_human.npy',     avg_return_cau)
        np.save('./data_tmp/data_ireca_return_AI.npy',     avg_return_ent)
        # np.save('./data_tmp/data_ireca_return_env.npy',     avg_return_env)
        # np.save('./data_tmp/data_ireca_return_ireca.npy',    avg_return_ireca)


if bc_model_path_train == "./bc_runs_ccima/reproduce_train/asymmetric_advantages":
