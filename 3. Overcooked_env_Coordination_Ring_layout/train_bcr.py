...
if bc_model_path_train == "./bc_runs_ccima/reproduce_train/coordination_ring":
    ...
    coeff_reward_cau = coeff_reward_cau_co
    coeff_reward_ent = coeff_reward_ent_co
...
for epoch in range(epochs):
    ...
    print(f" [caci] Epoch: ({epoch}). Mean Length: ({mean_length}).")
    ...
    if ((epoch+1) % 20 == 0) or ((epoch+1) == epochs) :
        np.save('./data_tmp/data_caci_return_sparse.npy',  avg_return_sparse)
        np.save('./data_tmp/data_caci_return_stage.npy',  avg_return_shaped)
        np.save('./data_tmp/data_caci_return_human.npy',     avg_return_cau)
        np.save('./data_tmp/data_caci_return_AI.npy',     avg_return_ent)
        # np.save('./data_tmp/data_caci_return_env.npy',     avg_return_env)
        # np.save('./data_tmp/data_caci_return_caci.npy',    avg_return_caci)


if bc_model_path_train == "./bc_runs_ccima/reproduce_train/coordination_ring":
    actor.save_weights("model_co_actor_caci.h5")
    critic.save_weights("model_co_critic_caci.h5")
