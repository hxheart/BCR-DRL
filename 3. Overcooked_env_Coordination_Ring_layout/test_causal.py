...
if bc_model_path_train == "./bc_runs_ccima/reproduce_train/coordination_ring":
    policy_optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate_policy_co)
    value_optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate_value_co)
    learning_rate_reward_shaping = learning_rate_reward_shaping_co
    clip_ratio = clip_ratio_co
    gamma = gamma_co
    GAE_lam = GAE_lam_co
    iterations_train_policy = iterations_train_policy_co
    # --
    actor.load_weights("./data/model/model_co_actor_cippo_5.h5")
    critic.load_weights("./data/model/model_co_critic_cippo_5.h5")
    epochs = int(400)
    # --
    print("\n loaded the coordination_ring weights \n")
...
episode_return_cau, episode_return_cippo = 0, 0
episode_length = 0
count_step = 0
...
for epoch in range(epochs):
    ...
    for t in range(steps_per_epoch):
        ...
        observation_HM_know_AI_action = tf.reshape(obs_both_agent_know_AI_action[1-other_agent_env_idx], (1,-1)) # 得到只有 ai agent 行动以后的 HM 的 observation
...
#         action_HM_agent = tf.argmax(bc_model(observation_HM, training=True), axis=1) # 不用随机策略
        ...
        reward_cippo = reward_env + 0.01*reward_cau
        ...
        episode_return_cippo += reward_cippo
        episode_length += 1
...
#         buffer.store(observation_AI, action_AI_agent, reward_env, value_t, logprobability_t)
        ...
        if terminal or (t == steps_per_epoch - 1):
            ...
            sum_return_cippo += episode_return_cippo
    ...
    print(f" [cippo] Epoch: {epoch}. Mean Length: {sum_length / num_episodes}")
    print(f" Mean sparse: {sum_return_sparse / num_episodes}. Mean shaped: {sum_return_shaped / num_episodes}. Mean Env: {sum_return_env / num_episodes}. Mean cau: {sum_return_cau / num_episodes}. Mean cippo: {sum_return_cippo / num_episodes}.")
    ...
    avg_return_cippo.append(sum_return_cippo / num_episodes)

    if ((epoch+1) % 20 == 0) or ((epoch+1) == epochs) :
        np.save('./data_tmp/data_cippo_return_stage.npy', avg_return_shaped)
        np.save('./data_tmp/data_cippo_return_sparse.npy', avg_return_sparse)
        # np.save('./data_tmp/data_cippo_return_env.npy',    avg_return_env)
        np.save('./data_tmp/data_cippo_return_causal_reward.npy',    avg_return_cau)
        # np.save('./data_tmp/data_cippo_return_cippo.npy',  avg_return_cippo)
...
# plt.savefig('./figs/cippo_avg_return_env.pdf', format='pdf')  
